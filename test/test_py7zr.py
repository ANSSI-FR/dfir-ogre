...
class TestPy7zr(TestCase):
    ...
    def test_stream(self):
        ...
        with py7zr.SevenZipFile(archive, "r", password="password") as archive:
            archive.extract(
                extract_path, targets=["folder/lorem.txt", "hello.txt"], factory=factory
            )

        with open(os.path.join(extract_path, "folder", "lorem.txt")) as f:
            content = f.read()
            assert "mollit anim id est laborum." in content
            self.assertEqual(len(content), 447)

        with open(os.path.join(extract_path, "hello.txt")) as f:
            content = f.read()
